...
export default class FlowCombobox extends LightningElement {
  ...
  set value(newValue) {
    this.isDataSelected = !!newValue;
    if (isReference(newValue)) {
      this._dataType = getDataType(newValue);
      this._value = removeFormatting(newValue);
    } else {
    ...
    switch (value) {
      ...
      case "$User":
        ...
        getObjectFields({ objectName: objectName }).then((result) => {
          ...
          // Set the Options
          this.setOptions([
            { type: value + " Outputs", options: tempOptions }
          ]);
        });
        break;
      case "$System":
